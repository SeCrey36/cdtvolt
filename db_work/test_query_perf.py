"""Run typical query, capture EXPLAIN PLAN, measure time, create indexes and repeat.

Usage:
  python db_work/test_query_perf.py

Outputs results to `db_work/perf_results.json`.
"""
import sqlite3
...
# Typical query: joins enrollments -> enrollment_timeslots -> timeslots -> courses
TYPICAL_QUERY = '''
...
INDEX_STEPS = [
    # step name, list of CREATE INDEX statements
    ('baseline', []),
...
def main():
    if not DB.exists():
        raise SystemExit(f'Database not found at {DB}; create and populate it first')
    ...
    for step_name, stmts in INDEX_STEPS:
        ...
        print(f"Step {step_name}: time={elapsed:.4f}s rows={rows}")
